...
class SearchInvoice extends Invoice
{
    ...
    public function rules()
    {
        return [
            ...
            [['invoice_no', 'date_issue', 'remarks', 'created_at','plate','date_a','date_b', 'updated_at','customer_name'], 'safe'],
            [['grand_total'], 'number'],
        ];
    }
    ...
    public function carplateSearch($params){
      ...
      $this->load($params);
       ...
       $query->select('inv.id, inv.invoice_no, inv.date_issue, inv.branch_id, inv.user_id,b.name as branch_name,b.code, u.username,u.fullname, ci.carplate, c.fullname as name,  inv.grand_total, inv.come_in, inv.come_out,inv.remarks')
              ->from('invoice inv')
              ->innerjoin('branch b', 'b.id=inv.branch_id')
              ->innerjoin('user u', 'u.id=inv.user_id')
              ->innerjoin('car_information ci', 'ci.id=inv.customer_id')
